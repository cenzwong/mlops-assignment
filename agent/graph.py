...
def _extract_json(text: str) -> dict:
    """Pull a JSON object out of an LLM response content, handling fences and prose."""
    # Find any JSON-like block enclosed in markdown fences
    ...
    # Try direct load of content
    try:
        return json.loads(content.strip())
    except Exception:
        pass
        
    # As a fallback, try regex searching for "ok": true/false and "issue": "..."
    ok_match = re.search(r'"ok"\s*:\s*(true|false)', text, re.IGNORECASE)
    issue_match = re.search(r'"issue"\s*:\s*"(.*?)"', text, re.DOTALL)
...
async def verify_node(state: AgentState) -> dict:
    ...
    in the README.
    """
    rows_str = state.execution.render() if state.execution else "No execution run"
    error_str = state.execution.error if (state.execution and not state.execution.ok) else "None"
    ...
    verify_ok = parsed.get("ok", False)
    verify_issue = parsed.get("issue", "")
    
    # If the SQL execution errored, ensure validation fails.
    if state.execution and not state.execution.ok:
        verify_ok = False
        if not verify_issue:
            verify_issue = f"SQL execution failed: {state.execution.error}"
